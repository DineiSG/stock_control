...
import RenderTable from '../outros/RenderTable';
...
const InstData = () => {
  ...
  const contarVenda = (venda) => {
    ...
    //Funçao que conta as transações de acordo com a quantidade de vezes que o nome dela aparece
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      return (
        <div style={{ backgroundColor: "white", border: "1px solid #ccc", padding: "10px", width: "150px", height: "80px", boxShadow: "0 0 10px rgba(0, 0, 0, 0.2 )" }}>
  ...
  return (
    <div className={styles.container_bar} >
      ...
      <p className={styles.p_txt}> Abaixo pode-se visualizar as quantidades de venda por instituicao. </p>
      ...
      <div className={styles.dateInputs}>
        ...
        <button className={styles.btn_buscar} onClick={fetchVendas} >Buscar</button>
      </div>
      {dadosGrafico.length > 0 ? (
        <ResponsiveContainer width="100%" height="80%">
          <BarChart
            ...
            height={500}
            data={dadosGrafico}
            margin={{ top: 5, right: 1, left: 5, bottom: 60, }} barSize={30}>
       ...
       null
            ...
            <div className={styles.render_table}>
