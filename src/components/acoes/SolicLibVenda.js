    ...
    if (textoMaiusculo.length === 7) {
      try {
        const response = await fetch(
          `http://localhost:8090/api/v1/veiculos/placa/${textoMaiusculo}`
        );
        if (response.ok) {
      ...
      <div>
        <h1>
          <img
            width="70"
            height="70"
            src="https://img.icons8.com/3d-fluency/94/car.png"
            alt="car"
          />{" "}
          Solicitação de Liberação de Veículo Por Venda
        </h1>
