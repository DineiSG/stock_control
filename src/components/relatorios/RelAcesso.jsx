  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      if (filteredResults.length > 0) {
        ...
        //Capturando a placa do veiculo e utilizando para buscar
        const idVeiculoAcessante = filteredResults[0].placa
        const relatedDataResponse = await fetch(`http://localhost:8090/api/v1/acessos/placa/${idVeiculoAcessante}`)
    ...
    setQuery('') //Limpa o campo de pesquisa apos a busca
    setFiltroAcesso(filtroAcesso) //Altera o estado da tabela para visivel
  }
      ...
      <div class="container-md" >
        <div className={styles.container}>
          <div className={styles.input}>
            {!filtroAcesso ? <h2 className={styles.title}>INFORME A PLACA DO VEÍCULO</h2>: <h2 className={styles.title}>RESULTADO DA BUSCA</h2>}
            <form className={styles.pesquisa} onSubmit={handleSearch}>
              ...
              <button className={styles.btn_buscar} type='submit' onClick={handleButtonClick}>{filtroAcesso ? 'Nova Busca' : 'Buscar'}</button>
            </form>
          </div>
        </div>
      </div>
